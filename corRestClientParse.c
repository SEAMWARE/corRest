...
// -----------------------------------------------------------------------------
//
// corRestClientResponseComplete - Read-only check: is the full response in the buffer?
//
...
//
static int parseStatusLine(char* line, int lineLen, CorRestClientResponse* resp)
{
  char* p   = line;
...
//
static void addResponseHeader(CorRestClientResponse* resp, char* name, char* value)
{
  if (resp->headerCount >= resp->headerSize)
  {
    int newSize = resp->headerSize + COR_REST_KV_GROW_SIZE;
    CorRestKeyValue* newV = (CorRestKeyValue*)malloc(newSize * sizeof(CorRestKeyValue));
    if (newV == NULL)
      return;

    memcpy(newV, resp->headerV, resp->headerCount * sizeof(CorRestKeyValue));
...
//
static int parseResponseHeader(char* line, int lineLen, CorRestClientResponse* resp,
                               int* contentLengthP, bool* chunkedP, bool* keepAliveP)
{
  char* colon = (char*)memchr(line, ':', lineLen);
  if (colon == NULL)
